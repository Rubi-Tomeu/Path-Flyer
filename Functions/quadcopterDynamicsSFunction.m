...
% Compute Drag forces
%
% Radius=0.1; %Rotor radius
% S = 0.0199; %Body Surface;
% Cd = 0.3; %AscTec Hummingbird body drag coefficient
% rho = 1.225; %Air density
% 
% 0.5*Cd*rho*S = 0.0037

% Wind disturbance velocity
wind_b = Dist_Fb;
% relative velocities
Ur = -wind_b(1) + U;
Vr = -wind_b(2) + V;
Wr = -wind_b(3) + W;
% força fregament
% Fd = 1.443169125242812e-01*[U*abs(U);V*abs(V);W*abs(W)]; %%Abans
Fd = 0.0037*[Ur*abs(Ur);Vr*abs(Vr);Wr*abs(Wr)];

% Compute Velocity and Position derivatives of body frame
vb = [U;V;W];
% b_dv = (1/quad.mass)*Fb+gb+Dist_Fb-OMb_bi*vb-Fd; %%Abans % Acceleration in body frame (FOR VELOCITY)
b_dv = (1/quad.mass)*(Fb-Fd)+gb-OMb_bi*vb;
